...
export const MESSAGE_TEMPLATES: Record<string, string> = {
  ...
  [MESSAGE_KEYS.EMAIL_SUBSCRIPTION.INVALID_EMAIL_FORMAT]: 'Định dạng email không hợp lệ',
  ...
  [MESSAGE_KEYS.VALIDATION.INVALID_PHONE]: 'Định dạng số điện thoại không hợp lệ',
  ...
  [MESSAGE_KEYS.VALIDATION.MAX_LENGTH]: '{field} không được vượt quá {max} ký tự',
  ...
  [MESSAGE_KEYS.FILTER_AND_SORT.INVALID_CATEGORY_ID]: 'ID danh mục không hợp lệ',
...
export function formatMessage(key: string, params?: Record<string, any>): string {
  let message = MESSAGE_TEMPLATES[key] || key;
